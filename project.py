...
GAME_STATE = "TOP_DOWN"  # Possible states: "TOP_DOWN", "PLAYING", "GAME_OVER", "GAME_WON"
game_over_timer = 0
TOP_DOWN_VIEW_TIME = 10000  # 10 seconds in milliseconds
start_time = 0
...
def generate_maze(rows, cols):
    ...
    preset_colors = [
        ...
        (0.2, 0.2, 0.9),  # Bright Blue
        (0.4, 0.1, 0.1),  # Dark Red
...
def init_game():
    global player_pos, player_angle, player_coins, player_bullets, enemies, coins, bullets, GAME_STATE, start_time, maze
    ...
    for r in range(len(maze)):
        for c in range(len(maze[0])):
            if maze[r][c] == 1:
                ...
                # Top face border
                # Draw all cube edges using GL_LINES
                glBegin(GL_LINES)
                # Bottom face edges
...
def draw_player():
    ...
    glutSolidCube(PLAYER_RADIUS * 2) # Use a cube to represent player for now
    glPopMatrix()
...
def update_game_logic():
    global GAME_STATE, game_over_timer, player_bullets, player_coins, maze
